...
public class ProductServiceImpl extends ServiceImpl<ProductDao, Product> implements IProductService {
    ...
    public Product addProduct(Product product) throws ProductAdminException {
        if(product.getId() < 0) {
            throw new ProductAdminException(ProductAdminException.INPUT_ERROR, "输入的id为负数");
        }
        try {
            this.baseMapper.insert(product);
        } catch (Exception e) {
            throw new ProductAdminException(ProductAdminException.DEFAULT_ERROR, "添加产品失败：" + e.getMessage());
        }
        return product;
    }
    ...
    public void updateProduct(long id, Product product) throws ProductAdminException {
        if (this.baseMapper.selectById(id) == null) {
            throw new ProductAdminException(ProductAdminException.EXIST_ERROR, "产品" + id + "不存在");
        }
        try {
            this.baseMapper.updateById(product);
        } catch (Exception e) {
            throw new ProductAdminException(ProductAdminException.DEFAULT_ERROR, "修改产品失败：" + e.getMessage());
        }
    }
    ...
    public void deleteProduct(long id) throws ProductAdminException {
        try {
            ...
            throw new ProductAdminException(ProductAdminException.DEFAULT_ERROR, "删除产品失败：" + e.getMessage());
        }
    }
}
